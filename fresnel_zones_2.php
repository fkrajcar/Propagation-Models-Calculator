<?php 
    ...
    $wf_sel2 = $_POST['wff_2'];
    $freq_sel2 = $_POST['freq_si_2'];
    $wf_sel3 = $_POST['wff_3'];
    $freq_sel3 = $_POST['freq_si_3'];
    ...
    if ($wf_sel2 == "1"){ //ovisno jel odabrana f ili lambda
        switch($freq_sel2) //pretvaranje u metre
        {
            case 'MHz': //pretvori u cm
            $lambda2 = $freq2/100;
            break;
        ...
        switch($freq_sel2) // pretvorba u Hz
        {
            case 'MHz':
            $freq2 = doubleval($_POST['freq_2']*1000000);
            break;

            case 'GHz':
            $freq2 = doubleval($_POST['freq_2']*1000000000);
            break;

            case 'Hz':
            $freq2 = doubleval($_POST['freq_2']);
            break;
        }

        $lambda2 = 300000000 / $freq2;
    }

        if ($wf_sel3 == "1"){ //ovisno jel odabrana f ili lambda
        switch($freq_sel3) //pretvaranje u metre
        {
            case 'MHz': //pretvori u cm
            $lambda3 = $freq3/100;
            break;
        ...
        switch($freq_sel3) // pretvorba u Hz
        {
            case 'MHz':
            $freq3= doubleval($_POST['freq_3']*1000000);
            break;

            case 'GHz':
            $freq3 = doubleval($_POST['freq_3']*1000000000);
            break;

            case 'Hz':
            $freq3 = doubleval($_POST['freq_3']);
            break;
        }

        $lambda3 = 300000000 / $freq3;
    }
    ...
    for($i = $brojac; $i < $d_tocke; $i=$i+$brojac){
            $y = sqrt( (($n)*$lambda2*$i*($test-$i))/($i+($test-$i)) );
            array_push($dataPoints2, array($i, $y));
    }


    for($i = $brojac; $i < $d_tocke; $i=$i+$brojac){
            $y = sqrt( (($n)*$lambda3*$i*($test-$i))/($i+($test-$i)) );
